              ...
              {calendarDays.map((d, i) => {
                return (
                  <div key={i} 
                    ...
                    className={`bg-white ${viewMode === 'week' ? 'min-h-[250px]' : 'min-h-[100px] cursor-pointer hover:shadow-md hover:z-10 relative'} flex flex-col p-2 transition-all ${
                    !d.isCurrentMonth ? 'opacity-50 bg-gray-50' : 
                    ...
                    <div className={`flex-1 flex ${viewMode === 'week' ? 'flex-col gap-2 overflow-y-auto pr-1 custom-scrollbar' : 'flex-col gap-1'}`}>
                      {SHIFTS.map(shiftInfo => {
                        ...
                        if (viewMode === 'month') {
                          ...
                          return (
                            <div key={shiftInfo.id} className={`flex items-center justify-between px-1.5 py-0.5 rounded text-[10px] font-bold ${
                              isUncovered 
                        ...
                        return (
                          <div key={shiftInfo.id} className={`flex flex-col gap-1 p-2 rounded border ${
                            isMorning ? 'bg-sky-50/50 border-sky-100' : 
                            isAfternoon ? 'bg-amber-50/50 border-amber-100' : 
                            'bg-indigo-50/50 border-indigo-100'
                          }`}>
                            <div className="flex items-center justify-between gap-1">
                              <span className={`text-[10px] font-black uppercase tracking-wider truncate ${
                                isMorning ? 'text-sky-700' : isAfternoon ? 'text-amber-700' : 'text-indigo-700'
                              }`}>
                                {shiftInfo.id}
                              </span>
                            ...
                            {staffList.length === 0 ? (
                              ...
                              <div className="flex flex-col gap-1 mt-1">
                                {staffList.map((staff, idx) => (
                                  <div key={idx} className={`text-[11px] font-bold p-1.5 rounded border-l-2 shadow-sm bg-white flex items-center gap-2 ${
                                    ...
                                    <span className="truncate">{staff?.fullName || 'Unknown'}</span>
                                  </div>
                                ))}
                              </div>
                            )}
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3 justify-end pt-4">
                ...
                <button 
                  ...
                  className={`px-5 py-2.5 text-white rounded-lg text-sm font-bold transition-colors flex items-center gap-2 shadow-sm uppercase tracking-wider disabled:opacity-50 disabled:cursor-not-allowed ${
                  ...
                  <div key={shiftInfo.id} className="bg-gray-50 border border-gray-100 rounded-xl p-4">
                    ...
                    {staffList.length === 0 ? (
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {staffList.map((staff, idx) => (
                          <div key={idx} className={`text-xs font-bold p-2.5 rounded-lg border-l-4 shadow-sm bg-white flex items-center gap-3 ${
                            isMorning ? 'border-sky-400 text-sky-900' : 
                            isAfternoon ? 'border-amber-400 text-amber-900' : 
                            'border-indigo-400 text-indigo-900'
                          }`}>
                            ...
                            <span className="truncate" title={staff?.fullName}>{staff?.fullName || 'Unknown'}</span>
                          </div>
                        ))}
                      </div>
                    )}
